                ...
                foreach (Vessel vessel in FlightGlobals.Vessels)
                {
                        ...
                        vessel.vesselType == VesselType.EVA)
                    {
                        ...
                        if (IFI_Crew > 0.0)
                        {
                            ...
                            Log.Info("After Atmo adjust, LS_Use: " + LS_Use + ",   IFI_Crew: " + IFI_Crew + ",   Elapsed_Time: " + Elapesed_Time);
                            ...
                            if (lastTimeUpdate == -1)
                                ...
                                timeDiff = Planetarium.GetUniversalTime() - lastTimeUpdate;
                            lastTimeUpdate = Planetarium.GetUniversalTime();
                            if (LS_Use > 0.0)
                            {
                       
                                double rtest = timeDiff * IFIUSEResources(Constants.LIFESUPPORT, vessel, vessel.loaded, LS_Use, IFI_Crew);
                                double stest = timeDiff * IFIUSEResources(Constants.SLURRY, vessel, vessel.loaded, -rtest, IFI_Crew);
